...
// the test needs to start ``server'' first
...
void SignHandler(int iSignNo) {
    /* reset the timer in the SignHandler. Caution here: the signHandler */
    struct itimerspec ts;

    int ret;

    ts.it_interval.tv_sec = 0;
    ts.it_interval.tv_nsec = 0;              // set to zero, i.e. the timer will trigger only once
    ts.it_value.tv_sec = 0;
    ts.it_value.tv_nsec = 100000*(count+1);                  // set the time of initial expiration
...
void testTimerSign() {
    ...
    ts_set_begin = rdtsc();
    ts.it_interval.tv_sec = 0;
    ts.it_interval.tv_nsec = 0;              // set the interval of expirations
    ts.it_value.tv_sec = 0;
    ts.it_value.tv_nsec = 10000;                  // set the time of initial expiration
    ret = timer_settime(timer, 0, &ts, NULL);  // arm the timer
    // logTime("timer set");
    ts_set_end = rdtsc();
